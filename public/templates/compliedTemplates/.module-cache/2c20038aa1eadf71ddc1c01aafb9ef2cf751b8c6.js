					...
					if(imgNode.length) {
						...
						if(localStorage && localStorage.message !== obj.message) {
							if(obj.message.fileUpload) {
								fileLink.attr('href',obj.message.fileUpload).attr('download',obj.message.fileUploadType);
								fileLink.text(obj.message.fileUploadType);
		...
		if(e['keyCode'] === 13 || e['keyCode'] === undefined) {
			ActionEvents.sendMessage({
				...
				message:{
					...
					fileUpload:localStorage.getItem('fileUploadUrl')|| null,
