...
var headerView = React.createClass({displayName: "headerView",
	...
	handleClick:function(e){
		...
		switch(reqType) {
			case  "login":
				...
				React.render(React.createElement(LoginView, null),document.getElementById('display-section'))
				return;
			case  "register":
				var Comp = require('./formView');
				React.render(React.createElement(Comp, {data: "register"}),document.getElementById('display-section'))
				return;
			case  "profile":
				var Comp = require('./formView');
				React.render(React.createElement(Comp, {data: "profile"}),document.getElementById('display-section'))
				return;
			case "myPage":
				var Comp = require('./myPage');
				React.render(React.createElement(Comp, null),document.getElementById('display-section'))
			case "logout":
				$.ajax({
					url:'/logout',
					datatype:'json',
	...
	render:function(){
		return(
			React.createElement("header", {className: "row header-section"}, 
				React.createElement("div", {className: "left col-lg-6 col-md-6 col-sm-6 col-xs-6 icon-section"}, 
					"icon"
				), 
				React.createElement("div", {className: "right col-lg-6 col-md-6 col-sm-6 col-xs-6 main-list-sec"}, 
					React.createElement("ul", {onClick: this.handleClick, className: "options-list"}, 
