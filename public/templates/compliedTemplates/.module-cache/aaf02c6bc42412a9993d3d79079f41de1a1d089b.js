		...
		if(e.currentTarget.files.length>0) {
			...
			fileReader.onload= function(file) {
				if(fileType !== 'img') {
					...
					fileData = fileReader[fileReader.length-fileReader.length] + encodeURIComponent(fileData[fileReader.length-1]);
					fileLink.attr('href','http://localhost:3000/'+fileData).attr('target','_blank');
